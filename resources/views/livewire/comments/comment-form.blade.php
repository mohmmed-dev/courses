<div>
    ...
    <form class="flex items-center gap-x-1" wire:submit='save' action="#form-comment">
            <div class="w-3/4">
                <input type="text" wire:model="body" name="body" id="body" value="{{old('body')}}" required class="bg-gray-50  border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="{{__("Desecration")}}">
                @error('body')
                    <p class="text-xl text-red-500">{{$message}}</p>
                @enderror
            </div>
            <button type="submit" class="btn btn-neutral">{{__("Save")}}</button>
